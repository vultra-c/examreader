...
data class ConnectionRequiredDialogState(
    val title: String = "请先连接手环",
    val message: String = "当前操作需要先连接手环后才能继续。",
...
data class ImportState(
    ...
    val customRegexTemplates: List<RegexTemplate> = emptyList(),
    val selectedCustomRegexTemplateName: String? = null
) {
...
data class LargeChapterWarningState(
    val message: String
)
...
data class SyncReadingDataState(
    ...
    val failedBooks: Map<String, String> = emptyMap(),
    ...
    val selectedBooksForSync: Set<String> = emptySet()
)
...
data class BandStorageInfo(
    ...
    val showWarning: Boolean get() = hasValidData && StorageUtils.isStorageLow(actualAvailable) && !isLoading
}
