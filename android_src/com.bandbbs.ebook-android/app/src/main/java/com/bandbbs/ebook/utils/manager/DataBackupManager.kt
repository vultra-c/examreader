    ...
    suspend fun backupData(context: Context, uri: Uri, db: AppDatabase): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val timePrefs =
                    context.getSharedPreferences(READING_TIME_PREFS, Context.MODE_PRIVATE)
                ...
                val progressPrefs =
                    context.getSharedPreferences(CHAPTER_READER_PREFS, Context.MODE_PRIVATE)
                ...
                books.forEach { book ->
                    ...
                    if (lastReadChapterId != -1) {
                        ...
                        if (lastReadChapter != null) {
                            ...
                            bookProgress.put(
                                "lastReadTimestamp",
                                progressPrefs.getLong("last_read_timestamp_${book.id}", 0L)
                            )
                            ...
                            chapters.forEach { chapter ->
                                ...
                                if (progressPrefs.contains(indexKey)) {
                                    ...
                                    val offset = progressPrefs.getInt(
                                        "reading_position_offset_${chapter.id}",
                                        0
                                    )
                                    val cp = JSONObject()
                                ...
                                if (lastChapter != null) {
                                    ...
                                    if (bookProgress.has("lastReadTimestamp")) {
                                        editor.putLong(
                                            ...
                                            bookProgress.getLong("lastReadTimestamp")
                                        )
                                    }
                                }
                            }
                            ...
                            if (bookProgress.has("chapters")) {
                                ...
                                for (j in 0 until chaptersArray.length()) {
                                    ...
                                    if (targetChapter != null) {
                                        editor.putInt(
                                            "reading_position_index_${targetChapter.id}",
                                            cp.getInt("pageIndex")
                                        )
                                        editor.putInt(
                                            "reading_position_offset_${targetChapter.id}",
                                            cp.getInt("offset")
                                        )
                                    }
                                }
