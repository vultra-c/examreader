...
- 长文性能优化：无缝滚动模式采用**增量追加渲染**——全文按行建轻量索引，只渲染已读到的内容（滚到底自动追加下一批，DOM 节点数随阅读进度增长、与文件总长无关），长文可连续滚动且不再卡顿/崩溃
...
| 端 | 版本号 | 版本名 |
|------|--------|--------|
| 手环端 | 260820 | V26.8.2.BAND（包名 com.whyy.snapnotes；无缝模式彻底套用弦电子书 detail.ux 两段式实现：全文切段落块→按字符预算打包成段，DOM 只渲染当前段+下一段，滚到底 @scrollbottom 换段、滚到顶 @scrolltop 回退，换段按上一段实测高度补偿 scroll-top，自然流折行不再叠字，长文连续滚动不卡） |
